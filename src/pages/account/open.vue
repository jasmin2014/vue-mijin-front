        ...
        <div class="credit-form-inner open-form-inner">
          ...
          <div class="credit-tab-content">
              ...
              <el-form ref="openForm" :model="openForm" label-width="180px" :rules="openFormRules" :disabled="openDisabled">
                ...
                <div class="flex-start credit-branch-bank">
                  <el-form-item label="开户银行：" prop="bankName">
                    <mj-select v-model="openForm.bankName" :options="bankList" @kv-change="handleChangeBankName" :clearable="true"></mj-select>
                  </el-form-item>
                  <el-form-item prop="provinceCode" label-width="0">
                    <mj-select v-model="openForm.provinceCode" :region="'86'" ref="changeProvince" @kv-change="handleChangeBankProvince" :clearable="true"></mj-select>
                  </el-form-item>
                  <el-form-item prop="cityCode" label-width="0">
                    <mj-select v-model="openForm.cityCode" :region="openForm.provinceCode" @kv-change="handleChangeBankCity" :clearable="true"></mj-select>
                  </el-form-item>
                </div>
                ...
                <div class="flex-start credit-idCode credit-idCodes">
                  <div class="flex">
                    ...
                    <div>
                      <mj-upload v-model="seals"  :limit="1" :max-size="'200K'" :placeholder="'注：格式png、jpg、jpeg，上传公章需在空白纸上大小控制在200k以内'" :mode="'EDIT'" @success="handleUploadSealPics" @remove="handleRemoveSealPics"></mj-upload>
                ...
                <div class="flex-start credit-idCode credit-idCodes mt20">
                  <div class="flex">
                    ...
                    <div>
                      <mj-upload v-model="receipts"  :limit="1" :max-size="'10M'" :placeholder="'注：格式png、jpg、jpeg，大小控制在10M以内'" :mode="'EDIT'" @success="handleUploadReceiptsPics" @remove="handleRemoveReceiptPisc"></mj-upload>
            ...
            <div class="credit-tab-pane" v-show="activeTab === PERSONAL">
              <el-form ref="openPersonalForm" :model="openPersonalForm" label-width="170px" :rules="openPersonalFormRules" :disabled="personalDisabled">
                ...
                <div class="flex-start credit-branchBank-address">
                  <el-form-item label="支行地点：" prop="provinceCodePersonal">
                    <div class="width450_2">
                      <mj-select v-model="openPersonalForm.provinceCodePersonal" :region="'86'" @kv-change="handleChangeBankProvince" :clearable="true"></mj-select>
                    </div>
                  </el-form-item>
                  <el-form-item prop="cityCodePersonal" label-width="0">
                    <div class="width450_2">
                      <mj-select v-model="openPersonalForm.cityCodePersonal" :region="openPersonalForm.provinceCodePersonal" @kv-change="handleChangeBankCity" :clearable="true"></mj-select>
                    </div>
                  </el-form-item>
                </div>
                <!--<div class="step-agreement credit-agree flex-start">
...
export default {
  ...
  data () {
    return {
      ...
      openFormRules: {
        'name': [
          {required: true, min:4, max:50, message: '请输入4到50个字符的企业名称', trigger: 'blur'},
          {pattern: this.$valid.errorChar, message: '输入有误，不允许输入特殊字符', trigger: 'blur'},
        ],
        'identityCode': [
        ...
        'legalName': [
          ...
          {pattern: this.$valid.errorChar, message: '输入有误，不允许输入特殊字符', trigger: 'blur'},
          {min: 1, max: 10, message: '2-10字以内', trigger: 'blur'}
        ],
        'legalIdentityCode': [
        ...
        'branchName': [
          {required: true,min: 0, max: 50, message: '请输入50个字符以内的支行名称', trigger: 'blur'},
          {pattern: this.$valid.errorChar, message: '输入有误，不允许输入特殊字符', trigger: 'blur'},
        ],
        'contactName': [
          {required: true,min: 2, max: 20, message: '请输入联系人姓名,不能超过20个字符', trigger: 'blur'},
          {pattern: this.$valid.errorChar, message: '输入有误，不允许输入特殊字符', trigger: 'blur'}
        ],
        'contactPhone': [
  ...
  computed: {
    seals: {
      get() {
        let _seals = [], currentSeal = [];
        if(this.openForm.proofMaterialDTOList && this.openForm.proofMaterialDTOList.length > 0) {
          _seals = this.openForm.proofMaterialDTOList.filter(_ => _.kind === this.$enum.COM_COMPANY_SEAL).map(_ => ({
            name: _.name,
            url: _.fileUri,
            type: _.fileType
          }));
    ...
    receipts: {
      get() {
        let _receipts = [] , currentReceipts = [];
        if(this.openForm.proofMaterialDTOList && this.openForm.proofMaterialDTOList.length > 0) {
          _receipts = this.openForm.proofMaterialDTOList.filter(_ => _.kind === this.$enum.COM_RETURN_RECEIPT).map(_ => ({
            name: _.name,
            url: _.fileUri,
            type: _.fileType
          }));
  ...
  methods: {
    getFormType(data){
      if(data.creditAmount <= this.totalLimit){//小于等于20w
        if(data.accountType === '1'){//已有开户状态切对公
          this.activeTab = this.ENTERPRISE; 
        }else{
          //对私
          this.activeTab = this.PERSONAL;
        }
      }else if(data.creditAmount > this.totalLimit){//20w+
        // //开户类型
        if(data.accountType === '0'){//对私
          this.activeTab = this.PERSONAL;
        }else if(data.accountType === '1'){//对公
          this.activeTab = this.ENTERPRISE;
        }else{
          //无accountType == -1或其他
          //放款类型第三方只能对公
          if(data.paymentType == this.THIRD_PARTY){
            this.activeTab = this.ENTERPRISE;
          }else if(data.paymentType == this.FINANCIAL){//财务放款对公对私自由显示
    ...
    getBankList(){
      api.getBankList().then(res => {
        if(res.data.code === 200) {
          this.bankList = res.data.body.map(_ => ({text: _.bankName, value: _.bankCode}));
        }
      })
    },
        ...
        if(res.data.code === 200) {
          if(res.data.body.status === '0') {
            ...
            this.openForm = {
              ...
              legalIdentityCode: _params.legalIdentityCode ? _params.legalIdentityCode: '',
              cardNo: _params.cardNo ? _params.cardNo : '',
              provinceCode: _params.provinceCode ? _params.provinceCode : this.openForm.provinceCode,
              bankProvince: _params.bankProvince ? _params.bankProvince : this.openForm.bankProvince,
              bankCity: _params.bankCity ? _params.bankCity : this.openForm.bankCity,
              cityCode: _params.cityCode ? _params.cityCode : this.openForm.cityCode,
              ...
              proofMaterialDTOList: _params.proofMaterialDTOList ? _params.proofMaterialDTOList : []
            };
            ...
            this.openForm.bankProvince = _params.bankProvince && _params.bankProvince !== '' ? this.getProvinceAndCityName(_params.bankProvince) : '';
            this.openForm.bankCity = _params.cityCode && _params.cityCode !=='' ? this.getProvinceAndCityName(this.openForm.cityCode) : '';
            ...
            this.openPersonalForm ={
              ...
              provinceCodePersonal: _params.provinceCode ? _params.provinceCode : this.openPersonalForm.provinceCodePersonal,
              cityCodePersonal: _params.cityCode ? _params.cityCode : this.openPersonalForm.cityCodePersonal,
              bankProvincePersonal: _params.bankProvince ? _params.bankProvince : this.openPersonalForm.bankProvincePersonal,
              bankCityPersonal: _params.bankCity ? _params.bankCity : this.openPersonalForm.bankCityPersonal
            };
            if(_params.proofMaterialDTOList && _params.proofMaterialDTOList.length > 0) {
              let _pSeals = [],_preceipts = [];
            ...
            this.openPersonalForm.bankProvincePersonal = _params.provinceCode && _params.provinceCode !== '' ? this.getProvinceAndCityName(_params.provinceCode) : '';
            this.openPersonalForm.bankCityPersonal = _params.cityCode && _params.cityCode !=='' ? this.getProvinceAndCityName(_params.cityCode) : '';
          }
        }
      })
    },
      ...
      this.$refs['openForm'].validate((valid) => {
        if (valid) {
          if(!this.sealIsNull && !this.receiptIsNull) {
            this.$confirm('当您完成企业开户后，您的账户性质将被定义为企业，此后将只能申请企业类产品，无法申请个人类产品, 是否继续?', '提示', {
    ...
    handleOpenAccount(){
      ...
      _params = {
        ...
        bankProvince: personParams.bankProvincePersonal ? personParams.bankProvincePersonal : '',
        bankCity: personParams.bankCityPersonal ? personParams.bankCityPersonal : '',
        returnUrl: this.$url.localURL + '/success'
      };
      this.$refs['openPersonalForm'].validate((valid) => {
        if (valid) {
          this.$confirm('当您完成个人开户后，您的账户性质将被定义为个人，此后将只能申请个人类产品，无法申请企业类产品, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
          }).then(() => {
            setTimeout(() => {
    ...
    openAccount(params){
      api.accountOpen(params).then(res => {
        if(res.data.code === 200) {
          if(res.data.body && res.data.body !== null) {
            window.location.href = res.data.body;
          }else{
            this.$router.push({name:'successPage',query:{'pageName':'accountOpenPage'}})
          }
        }else{
          this.showDialog = true;
          this.isSuccess = false;
        ...
        if(response.data.body){
          ...
          this.messageInfo = response.data.message ? response.data.message : '开户失败！';
        }
      })
    },
    handleClose(){
