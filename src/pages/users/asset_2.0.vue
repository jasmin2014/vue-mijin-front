    ...
    <div class="users-content">
      <div class="main-content users-inner flex-start">
        ...
        <div class="main-right">
          <section class="asset-mr01 no-open" v-if="accountInfo.fundsStatus == 0">您暂未开通资金账户</section>
          <section class="asset-mr01 flex" v-else-if="accountInfo.fundsStatus == 1">
            <div>
              <p class="bankname">
                <!--<span class="sprites cmb"></span>-->
                {{accountInfo.bankName}}
                <span v-if="accountInfo.accountType === 0">对私</span>
                <span v-if="accountInfo.accountType === 1">对公</span>
              </p>
            ...
            <div class="amr02-seach-inner">
              <div class="search-tabs-header flex">
                <div
                  ...
                  class="cursor"
                  @click="activeItem = 'first';"
                >资金流水1</div>
                <div
                  :class="{'search-tab-active': activeItem === 'second'}"
                  class="cursor"
                  @click="activeItem = 'second';"
                >资金流水2</div>
              </div>
    ...
    getAccountBankInfo() {
      api.getAccountBankInfo().then(res => {
        if (res.data.code === 200) {
          ...
          this.accountInfo.bankName = res.data.body.bankName
            ? res.data.body.bankName
            : "";
          this.accountInfo.bankCardNo = res.data.body.bankCardNo
            ? res.data.body.bankCardNo
            : "";
          this.accountInfo.availableBalance = res.data.body.availableBalance
            ? res.data.body.availableBalance
            : 0;
          this.accountInfo.freezeAmount = res.data.body.freezeAmount
            ? res.data.body.freezeAmount
            : 0;
          this.accountInfo.status = res.data.body.status
