<template>
  <div class="content">
      ...
      <div class="main-content credit-form">
        ...
        <div class="credit-form-inner credit-form-guarantee">
          <div class="credit-form-toptext">
            <p v-if="pageFromName === 'usersCreditPage' && operateType === 'Change'" style="height: 40px;width: 100px;"></p>
            <!-- <p v-else>您的授信额度超过20万，需添加担保人信息！</p> -->
          </div>
          <el-form ref="guaranteeForm" :model="guaranteeForm" label-width="150px" :rules="guaranteeFormRules">
    ...
    <mijin-dialog v-show="showOpenDialog" :is-success="isOpenSuccess" :message="messageOpenInfo" :double-button="doubleButton" :ok-button-name="okButtonName" @ok="handleRedirectOpen" @close="handleNoOpenClose"></mijin-dialog>
  </div>
</template>
...
export default {
    ...
    return {
      ...
      guaranteeFormRules: {
        ...
        guaranteeName: [
          ...
          { pattern: this.$valid.errorChar , min:2, max: 20, message: '2～20个字，不可输入特殊字符和空格', trigger: 'blur'}
        ]
      }
    }
  },
  ...
  methods: {
    ...
    handleRedirectOpen(){
      ...
      if(this.$route.query.creditAmount && this.$route.query.creditAmount!=''){
        creditAmount=Number(this.$route.query.creditAmount)
      }
      this.getOpennedStatus( () => {
        this.$router.push({name:'accountOpenPage',query:{accountType: this.accountType,creditAmount:creditAmount}})
      });
    },
    postAddGuarantee(){
      ...
      this.$refs['guaranteeForm'].validate((valid) => {
        if (valid) {
          api.postAddGuarantee(params).then(res => {
            if(res.data.code === 201){
              this.handleNoOpenClose();
              this.$router.push({name:'successPage',query:{'pageName':'creditGuaranteePage'}})
            }
          },response => {
            if(response.data.code === 4002024){
              ...
              if(response.data.body){
                ...
                this.messageOpenInfo = response.data.message ? response.data.message : '添加担保人失败!';
              }
              this.doubleButton = false;
            }
    ...
    putChangeGuarantee(){
      ...
      userApi.putChangeGuarantee(this.currentcreditId,_params).then(res => {
        if(res.data.code === 200){
