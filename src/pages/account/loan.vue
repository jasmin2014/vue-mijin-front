          ...
          <div class="credit-form-inner loan-form-inner">
            ...
            <div class="loan-form-item">
              <el-form-item label="借款金额" prop="applicationAmount">
                <mj-input v-model="creditForm.applicationAmount" :placeholder="'至少10000，最多'+availableAmount" @blur="handleBlur"></mj-input>
              </el-form-item>
            </div>
            <el-form-item label="还款方式" class="loan-check-items" prop="repayWay">
              <ul v-model="creditForm.repayWay">
                <li v-for="item in strategyList" :class="{'activeItems': creditForm.repayWay === item.repayWay}" :key="item.kind" @click="handleSetCurrentRepayWay(item)">
                  <label></label>{{$filter(item.repayWay, $enum.REPAY_WAY, $enum.REPAY_WAY)}}{{item.plusName}}</li>
              </ul>
            </el-form-item>
            <el-form-item v-if="repayTermsList.length > 0" label="借款期限" class="loan-check-items" prop="repayTerms">
              <ul class="flex-start" v-model="creditForm.repayTerms">
                <li v-for="item in repayTermsList" :class="{'activeItems': creditForm.repayTerms === item}" :key="item" @click="handleSetCurrentRepayTerm(item)">
                  <label></label>{{item}}个月</li>
              </ul>
            </el-form-item>
            <el-form-item label="还款总额" class="loan-check-items">
              <span class="desc">
                <span class="total">{{repayTatol}}</span>元，具体还款额以实际账单为准</span>
...
<script>
  ...
  export default {
          ...
          'applicationAmount': [{
            ...
            },
            {
              ...
              validator: (rule, value, callback) => {
                if (this.$valid.integer.test(value) && Number(value) >= this.minLoanAmount && Number(value) <=
    ...
    created() {
      this.productName = this.$getSessionStorage('checkedProducts') ? JSON.parse(this.$getSessionStorage(
        'checkedProducts')).productName : '';
    ...
    methods: {
      ...
      getLoanInformation() {
        if (this.$getSessionStorage('checkedProducts')) {
          api.getLoanInformation(JSON.parse(this.$getSessionStorage('checkedProducts')).creditId).then(res => {
            if (res.data.code === 200) {
      ...
      handleBlur(val) {
        this.$setSessionStorage('applicationAmount', this.creditForm.applicationAmount);
      },
      handleChecked() {
          ...
          this.$refs['creditForm'].validate((valid) => {
            if (valid) {
              const params = Object.assign(this.creditForm, {
                creditId: this.$getSessionStorage('checkedProducts') ? JSON.parse(this.$getSessionStorage(
                  'checkedProducts')).creditId : this.creditId,
                productId: this.$getSessionStorage('checkedProducts') ? JSON.parse(this.$getSessionStorage(
                  'checkedProducts')).productId : this.productId,
                strategyId: this.strategyId ? this.strategyId : ''
              });
              api.postLoanApplication(params).then(res => {
                if (res.data.code === 201) {
                  ...
                  this.messageInfo = res.data.message ? res.data.message : '借款失败!';
                }
              }, response => {
                this.showMessageDialog = true;
                this.isSuccess = false;
