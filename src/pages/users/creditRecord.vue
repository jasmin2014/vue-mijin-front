                ...
                <div class="search-tabs-item">
                    ...
                    <el-form :model="search" class="amr02-flex flex">
                      ...
                      <div class="flex">
                        <el-form-item label="授信状态">
                          <mj-select v-model="search.status" :kind="$enum.CREDIT_STATUS" :group="$enum.CREDIT_GROUP" :clearable="true"></mj-select>
                        </el-form-item>
                  ...
                  <el-table
                    ...
                    <el-table-column
                      ...
                      label="操作">
                      <template slot-scope="scope">
                        <div class="operate" v-if="scope.row.needGuarantor"><span @click="handleSeeGuarantee(scope.row)">查看担保信息</span></div>
                        <span v-else></span>
                      </template>
                    </el-table-column>
                  </el-table>
    ...
    <div class="users-message-confirm" v-if="showGuaranteeDialog">
      ...
      <section class="confirm-content gua-dialog-info">
        ...
        <div class="confirm-inner" v-if="guaranteeDetail.show === 2">
            <div class="flex-start"><label>关联授信编号:</label><span>{{guaranteeDetail.creditId}}</span></div>
            <div class="flex-start"><label>关联产品名称:</label><span>{{guaranteeDetail.productName}}</span></div>
            <div class="flex-start"><label>担保金额:</label><span>{{guaranteeDetail.creditAmount}}</span></div>
            <div class="flex-start"><label>担保人姓名:</label><span>{{guaranteeDetail.guaranteeName}}</span></div>
            <div class="flex-start"><label>担保人手机号:</label><span>{{guaranteeDetail.guaranteeMobile}}</span></div>
            <div class="flex-start"><label>担保人状态:</label><span>{{$filter(guaranteeDetail.status, this.$enum.GUARANTEE_STATUS, this.$enum.GUARANTEE_GROUP)}}</span></div>
            <div class="flex-start" v-if="guaranteeDetail.guaranteeProtocolUrl && guaranteeDetail.guaranteeProtocolUrl !==''">
              <label>担保合同</label>
              <a :href="guaranteeDetail.guaranteeProtocolUrl" target="_blank" >查看合同</a>
            </div>
            <div class="confirm-btns">
              <div class="cursor" v-if="guaranteeDetail.status === 'FAIL' || guaranteeDetail.status === 'REFUSE' || guaranteeDetail.status === 'INVALID'" @click="handleChangeGuarantor">更换担保人</div>
              <div @click="showGuaranteeDialog = false;" class="gua-cancel cursor" style="margin-top: 20px;">取消</div>
            </div>
        </div>
  ...
  export default {
    ...
    methods: {
      handleGoAddGuarantor(){
        this.$router.push({name:'creditGuaranteePage', query:{pageName:'usersCreditPage',operateType:'Add',creditAmount:this.creditAmount}})
      },
      ...
      getCreditApplicationList(index){
        ...
        api.getCreditApplicationList(_search).then(res => {
          if(res.data.code === 200) {
            this.accountCreditDatas = res.data.body.list ? res.data.body.list: [];
      ...
      handleChangeGuarantor(){
        this.$router.push({name:'creditGuaranteePage',query:{pageName: 'usersCreditPage', creditId: this.currentCreditId, creditAmount: this.creditAmount, operateType: 'Change'}})
      },
