            ...
            <el-form ref="authBankForm" :model="authBankInfo" label-width="100px" :rules="rechargeFormRules">
              ...
              <el-form-item label="支付方式">
                <div class="mr-recharge-ways flex">
                  <div v-if="accountType === 0" :class="{'active-item':activeWay === 'P_D'}" class="cursor" @click="activeWay = 'P_D'">认证支付</div>
                  <div class="isDisable" v-else>认证支付</div>
                  <div :class="{'active-item':activeWay === 'P_W'}" class="cursor" @click="activeWay = 'P_W'">网银支付</div>
                </div>
              </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleCode() {
        api.postPayCertCheck(this.authBankInfo.mobile).then((res) => {
      ...
      handleSubmitRecharge() {
        ...
        this.$refs['authBankForm'].validate((valid) => {
          if (valid) {
            ...
            api.postAccountRecharge(_params).then(res => {
              if (res.data.code === 200) {
                if (res.data.body.execType === 'form') {
                  this.$confirm('是否确认充值?', '提示', {
                    ...
                    divElement.innerHTML = this.rechargeDatas;
              ...
              this.showMessageDialog = true;
              this.isSuccess = false;
              this.messageInfo = response.data.message ? response.data.message : '充值失败!';
            })
          } else {
            return false;
          }
