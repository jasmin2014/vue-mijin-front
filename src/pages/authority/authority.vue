<template>
  <div class="content">
      ...
      <div class="main-content credit-form">
        ...
        <!--<form>-->
          <div class="credit-form-inner auth-idcard-inner">
            <el-form ref="authorityform" :model="creditForm" label-width="150px" :rules="pageFrom !== 'usersGuaranteesPage' || operateType === 'AccountAuthority' ? creditFormRules : {}">
              <div v-if="pageFrom !== 'usersGuaranteesPage' ||  operateType === 'AccountAuthority'">
                ...
                <el-form-item class="custom-width" :class="{'isIdentDisable':isIdentDisable}" label="身份证号：" v-else>
                  <div class="width450">
              ...
              <div class="flex-start credit-idCode credit-idCodes mt20">
                <div class="flex authority-item">
                  <p class="special-label"><span>*</span>身份证正面照：</p>
                  <div class="auth-upload" v-if="hasIDcode_M">
                    <mj-upload v-model="IDcodes_pic" :limit="1" accept="image/*" :upload-title="'个人信息页'" :max-size="'10M'" :placeholder="tipMessage" @success="handleIDcodePositiveSuccess" @remove="handleIDcodePositiveRemove"></mj-upload>
                    <p class="pic-error-msg" v-if="isIDcodesNull"> {{IDcodeErrorMsg}} </p>
                  </div>
                  <div class="auth-upload" v-else>
                    <mj-upload v-model="IDcodes_new" :limit="1" accept="image/*" :upload-title="'个人信息页'" :max-size="'10M'" :placeholder="tipMessage" @success="handleIDcodePositiveSuccess" @remove="handleIDcodePositiveRemove"></mj-upload>
                    <p class="pic-error-msg" v-if="isIDcodesNull"> {{IDcodeErrorMsg}} </p>
                  </div>
                </div>
                <div class="upload-example">
                  <p class="upload-txt">查看示例</p>
                  <p><img src="../../assets/imgs/idCard-front.jpg"> </p>
                </div>
              </div>
              <div class="flex-start credit-idCode credit-idCodes mt20">
                <div  class="flex authority-item">
                  <p class="special-label"><span>*</span>身份证反面照：</p>
                  <div class="auth-upload" v-if="hasIDcodeBack_M">
                    <mj-upload v-model="IDcodeOtherSide_pic" accept="image/*" :limit="1" :upload-title="'国徽页'" :max-size="'5M'" :placeholder="tipMessage" @success="handleOtherSideSuccess" @remove="handleOtherSideRemove"></mj-upload>
                    <p class="pic-error-msg" v-if="isOtherSideNull"> {{OtherSideErrorMsg}} </p>
                  </div>
                  <div class="auth-upload" v-else>
                    <mj-upload v-model="IDcodeOtherSide_new" accept="image/*" :limit="1" :upload-title="'国徽页'" :max-size="'5M'" :placeholder="tipMessage" @success="handleOtherSideSuccess" @remove="handleOtherSideRemove"></mj-upload>
                    <p class="pic-error-msg" v-if="isOtherSideNull"> {{OtherSideErrorMsg}} </p>
                  </div>
                </div>
                <div class="upload-example">
                  <p class="upload-txt">查看示例</p>
                  <p><img src="../../assets/imgs/idCard-behind.jpg"> </p>
                </div>
              </div>
              <div class="flex-start credit-idCode credit-idCodes mt20">
                <div class="flex authority-item">
                  <p class="special-label"><span>*</span>手持身份证照：</p>
                  <div class="auth-upload" v-if="hasIDcodeHand_M">
                    <mj-upload v-model="IDhandle_pic" accept="image/*" :limit="1" :upload-title="' '" :max-size="'5M'" :placeholder="tipMessage" @success="handleIDcodeSuccess" @remove="handleIDcodeRemove"></mj-upload>
                    <p class="pic-error-msg" v-if="HandIDCodeNull"> {{HandIDCodeErrorMsg}} </p>
                  </div>
                  <div class="auth-upload" v-else>
                    <mj-upload v-model="handIDCode_new" accept="image/*" :limit="1" :upload-title="' '" :max-size="'5M'" :placeholder="tipMessage" @success="handleIDcodeSuccess" @remove="handleIDcodeRemove"></mj-upload>
                    <p class="pic-error-msg" v-if="HandIDCodeNull"> {{HandIDCodeErrorMsg}} </p>
                  </div>
                </div>
                <div class="upload-example">
                  <p class="upload-txt">查看示例</p>
                  <p><img src="../../assets/imgs/idCard-handle.jpg"> </p>
                </div>
              </div>
              <div v-if="pageFrom === 'usersGuaranteesPage' && operateType === 'Edit'" class="flex-start credit-idCode credit-reports authority-item mt20">
                <p class="special-label"><span>*</span>人行征信报告：</p>
                <div v-if="hasBankCredit_M">
                  <mj-upload v-model="reports_pic"  :limit="5"  @success="handleReportsSuccess" :max-size="'10M'" :placeholder="'支持png/pdf/jpg和jpeg格式，大小不超过10M,最多上传5张'" @remove="handleReportsRemove"></mj-upload>
                  <p class="pic-error-msg" v-if="ReportsNull"> {{ReportsErrorMsg}} </p>
                </div>
                <div v-else>
                  <mj-upload v-model="reports_new"  :limit="5"  @success="handleReportsSuccess" :max-size="'10M'" :placeholder="'支持png/pdf/jpg和jpeg格式，大小不超过10M,最多上传5张'" @remove="handleReportsRemove"></mj-upload>
                  <p class="pic-error-msg" v-if="ReportsNull"> {{ReportsErrorMsg}} </p>
                </div>
              </div>
              <div class="step-btns">
                <div class="mj-button orange step-btn-next cursor" v-if="pageFrom === 'usersGuaranteesPage' || pageFrom === 'usersAccountPage' " @click="handleIdentAuth">提交</div>
    ...
    <mijin-dialog v-show="showAuthDialog" :is-success="isSuccess" :message="messageInfo" :is-direct="isDirect" @ok="handleSuccessRedirect" @close="handleClose"></mijin-dialog>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      creditFormRules: {
        ...
        'bankCreditReports': [
          {required: true, message: '请上传人行征信报告'}
        ]
      }
    }
  },
  ...
  computed: {
    IDcodes_pic: {
      get() {
        let _IDcodePositive = [], currentIDcode = [];
        if(this.creditForm.proofMaterialDTOList && this.creditForm.proofMaterialDTOList.length > 0) {
          _IDcodePositive = this.creditForm.proofMaterialDTOList.filter(_ => _.kind === USER_IDENTITY).map(_ => ({
            name: _.name,
            url: _.fileUri,
            type: _.fileType
          }));
    ...
    IDcodeOtherSide_pic: {
      get() {
        let _IdOtherSide = [], currentIDOtherSide = [];
        if(this.creditForm.proofMaterialDTOList && this.creditForm.proofMaterialDTOList.length > 0) {
          _IdOtherSide = this.creditForm.proofMaterialDTOList.filter(_ => _.kind === USER_IDENTITY_BACK).map(_ => ({
            name: _.name,
            url: _.fileUri,
            type: _.fileType
          }));
    ...
    IDhandle_pic: {
      get() {
        let _Idhandle = [], currentIDhandle = [];
        if(this.creditForm.proofMaterialDTOList && this.creditForm.proofMaterialDTOList.length > 0) {
          _Idhandle = this.creditForm.proofMaterialDTOList.filter(_ => _.kind === HAND_USER_IDENTITY).map(_ => ({
            name: _.name,
            url: _.fileUri,
            type: _.fileType
          }));
    ...
    reports_pic: {
      get() {
        let _reports = [];
        if(this.creditForm.proofMaterialDTOList && this.creditForm.proofMaterialDTOList.length > 0) {
          _reports = this.creditForm.proofMaterialDTOList.filter(_ => _.kind === BANK_CREDIT_REPORT).map(_ => ({
            name: _.name,
            url: _.fileUri,
            type: _.fileType
          }));
  ...
  methods: {
      ...
      getLoginDetail().then(res => {
        if(res.data.code === 200) {
          ...
          if(_data.personSource && _data.personSource === 'FENGTAI') {
            ...
            this.creditForm.proofMaterialDTOList = _data.proofMaterialDTOList && _data.proofMaterialDTOList.length > 0 ? _data.proofMaterialDTOList : [];

            if(_data.proofMaterialDTOList && _data.proofMaterialDTOList.length > 0) {
              let _pics1 = [] , _pics2 = [], _pics3 = [], _pics4 = [];
    ...
    handleIdentAuth(){
      ...
      if(this.pageFrom === 'usersGuaranteesPage' && this.operateType === 'Edit'){
        ...
        if(!this.isIDcodesNull && !this.isOtherSideNull && !this.HandIDCodeNull && !this.ReportsNull) {
          this.$refs['authorityform'].validate((valid) => {
            if (valid) {
              const _appid = this.$route.query.applicationId;
              console.log("authMaterial_before:",authMaterial);
              authMaterial = authMaterial.filter(_ => _ !== null);
              console.log("authMaterial:",authMaterial);
              uapi.putAuthorityInfo(_appid,authMaterial).then(res => {
                if(res.data.code === 200) {
                ...
                if(response.data.body) {
                  ...
                  this.messageInfo = response.data.message ? response.data.message : '信息修改失败，请检查您提交的信息!'
                }
                this.showAuthDialog = true;
                this.isSuccess = false;
                this.isDirect = false;
        ...
        if(!this.isIDcodesNull && !this.isOtherSideNull && !this.HandIDCodeNull) {
          this.$refs['authorityform'].validate((valid) => {
            if (valid) {
              console.log("authMaterial_before:",authMaterial);
              authMaterial = authMaterial.filter(_ => _ !== null);
              ...
              Api.identAuth(_params).then(res => {
...
.credit-reports .upload .el-upload-list--picture-card .el-upload-list__item:nth-child(2n) {
  margin-right: 0;
}
  .authority-item {
    .special-label {
