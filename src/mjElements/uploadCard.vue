<template>
  <div class="mj-upload">
    ...
    >
      <template>
        <div class="text" :class="{'hasWarnMsg':picWarnMessage && picWarnMessage !==''}">
          <i class="icon iconfont icon-tianjia"></i>
          <span class="text-upload">添加{{picName}}</span>
          <span
            class="pic-warn-message"
            :class="{'warnMsgIsLong':picWarnMessage.length > 20}"
            v-if="picWarnMessage"
          >{{picWarnMessage}}</span>
          ...
          <div
            class="el-upload__placehol der"
            :class="{'warnMsgIsLong':picWarnMessage && picWarnMessage !=='' && picWarnMessage.length > 20}"
          >{{ placeholder }}</div>
        </div>
      </template>
    </el-upload>
...
.mj-upload {
  ...
  .el-upload__placehol.warnMsgIsLong {
    margin-top: 15px;
  }
}
...
.text {
  ...
  // span.pic-warn-message {
  //   color: #000;
  // }
  span.pic-warn-message.warnMsgIsLong {
    width: 90%;
    display: block;
    margin: 0 auto;
    line-height: 22px;
