...
export default {
  ...
  methods: {
    ...
    getOptions(kind, group, enumKey, visible, sequence) {
      ...
      api.getSysEnum(kind, group, enumKey, visible, binarySequence).then(
        response => {
          const res = response.data;
          if (res.code === 200) {
            res.body.forEach(_item => {
              if (_item.displayName === "调账-逾期减免")
                _item.displayName = "逾期减免";
            });
