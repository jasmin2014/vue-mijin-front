  ...
  <div class="content">
        ...
        <div class="main-content top-quota-content">
          <div class="main-content quota-box">
              ...
              <div class="quota-choose-items">
                ...
                <ul class="clearfix">
                  <li
                    ...
                    :key="index"
                    :class="{'active':activeProduct === product.productId}"
                    @click="handleCheckedProducts(product)"
                  >
                    <label></label>
                    {{product.productName}}
                  </li>
            ...
            <section class="quota-no" v-else>
              ...
              <div class="quota-btns">
                <div class="quota-product">
                  授信产品为
                  <span>{{creditAmount.productName }}</span>
                </div>
    ...
    <div class="mj-message-box" v-show="isNoLogin">
      ...
      <div class="message-inner message-login">
        <div class="message-close cursor" @click="isNoLogin=false;">
    ...
    <mijin-dialog
      v-show="showRedirectDialog"
      :is-success="isRedirectSuccess"
      :message="messageRedirectInfo"
...
export default {
  ...
  methods: {
          ...
          if (res.data.code === 200) {
            ...
            if (res.data.body.haveAmount) {
              ...
              this.creditAmount.creditAmount =
                Number(_data.creditAmountDTOList[0].creditAmount) >= 0
                  ? _data.creditAmountDTOList[0].creditAmount
                  : 0;
              this.creditAmount.availableAmount =
                Number(_data.creditAmountDTOList[0].availableAmount) >= 0
                  ? _data.creditAmountDTOList[0].availableAmount
                  : 0;
    ...
    handleCheckedProducts(row) {
      ...
      this.$setSessionStorage("productType", row.productType);
    },
    handleRedirect() {
          ...
          api.getCheckedCredit().then(
            res => {
              if (res.data.code === 200) {
                ...
                this.$setSessionStorage(
                  "checkedItems",
                  JSON.stringify(_creditUserInfo)
                );
                if (res.data.body == "1") {
                  MessageBox.confirm(
                    "尊敬的客户，该注册手机号已经开通过个人存管账户，如需进行企业借贷，烦请更换新手机号重新注册申请，谢谢配合！如有疑问请拨打客服电话4006690108",
                    "提示",
                    {
                  ...
                  MessageBox.confirm(
                    "尊敬的客户，该注册手机号已经开通过企业存管账户，如需进行个人借贷，烦请更换新手机号重新注册申请，谢谢配合！如有疑问请拨打客服电话4006690108",
                    "提示",
                    {
                      confirmButtonText: "继续申请",
                      cancelButtonText: "取消",
                ...
                } else if (res.data.body == "0" || res.data.body == "3") {
                  this.$router.push({
                    name: "creditApplyStep1",
                    query: { productType: this.productType }
                  });
    ...
    handleCheckedAccountOpen() {
      if (
        this.creditAmount.availableAmount &&
        Number(this.creditAmount.availableAmount) > 0
      ) {
        this.checkAccountFn();
      } else {
...
.top-banner-quota {
  ...
  .top-quota-inr {
    position: relative;
    height: 100%;
    background: url("../assets/imgs/banner/quota-bg_02.jpg") no-repeat center
      440px;
    .header {
      position: absolute;
      left: 0;
    ...
    .top-quota-content {
      height: 100%;
      background: url("../assets/imgs/banner/quota-pic_01.jpg") no-repeat center
        top;
      .quota-box {
        height: 100%;
        background: url("../assets/imgs/banner/quota-pic_02.jpg") no-repeat
          center 440px;
      }
    }
  }
