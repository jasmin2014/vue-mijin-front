  ...
  <div class="content">
    ...
    <div class="users-content">
      <div class="main-content flex-start">
        <div class="main-left">
          <mijin-left-menu :active-index="0"></mijin-left-menu>
        </div>
        <div class="main-right mr-recharge">
          <h3>提现到银行卡</h3>
          <div class="mr-recharge-content">
            <el-form ref="authBankForm" :model="authBankInfo" label-width="100px" :rules="rechargeFormRules">
              <el-form-item label="提现金额" prop="amount">
                <mj-input v-model="authBankInfo.amount" :unit="'元'"></mj-input>
                <div class="flex mr-withdraw-txt">
              ...
              <el-form-item label="已认证银行">
                <div class="flex mr-recharge-card">
                  <p><b>{{authBankInfo.bankName}}</b></p>
                  <p><b>尾号{{authBankInfo.bankCard}}</b></p>
                </div>
              ...
              <el-form-item label="短信验证码" prop="identifyCode">
                <div class="flex mr-recharge-code">
                  ...
                  <div class="mr-code-get cursor" :class="{'disabled-code' : disabledCode}" @click="handleCode(authBankInfo.phone)">
                    {{ btnCountDown}}</div>
                </div>
...
<script>
  ...
  export default {
      ...
      return {
        ...
        rechargeFormRules: {
          'amount': [
            ...
            {pattern: this.$valid.integer, message: '请输入正数，最多保留两位小数!', trigger: 'blur'},
            {
              trigger: 'blur',
    ...
    methods: {
      ...
      handleCode(phone){
        api.postWithdrawCheckCode(phone).then((res)=>{
          ...
          if(res.data.body) {
            ...
            this.messageInfo = res.data.message ? res.data.message : '获取短信验证码失败!'
          }
      ...
      getAuthBankInfo(){
        ...
        api.getAuthBankInfo(_params).then(res => {
          if(res.data.code === 200) {
            ...
            this.authBankInfo.phone = res.data.body.mobile ? res.data.body.mobile : '';
          }
        })
      },
      handleSubmitRecharge(){
        this.$refs['authBankForm'].validate((valid) => {
          if (valid) {
            ...
            api.postWithDrawForFund(_params).then(res => {
              if(res.data.code === 200) {
                this.$router.push({name:'successPage',query:{pageName:'usersWithdrawPage'}})
              }
            }, res => {
              if(res.data.body) {
                let _msg = '';
                ...
                this.messageInfo = res.data.message ? res.data.message : '提现失败!'
              }
              this.showMessageDialog = true;
              this.isSuccess = false;
